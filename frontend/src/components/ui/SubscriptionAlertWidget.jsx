...
//   return (
//     <>
//       {/* 1. Floating Bubble Button with Notification Count Badge */}
//       {/* Placed at bottom-right, shifted left to perfectly avoid overlapping the AI Chat widget */}
//       <button
//         onClick={() => setIsOpen(true)}
//         className={`fixed bottom-6 right-24 w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-transform hover:scale-105 z-40 animate-pulse
//           ${isOpen ? 'hidden' : 'flex'} bg-amber-500 text-white`}
//       >
...
//           ${dark ? 'bg-[#1A1A1A] border-[#2A2A2A]' : 'bg-orange-50 border-orange-100'}`}>
//           <div className="flex items-center gap-2">
//             <div className="w-8 h-8 rounded-full bg-amber-500 flex items-center justify-center">
//               <MdNotificationsActive className="text-white text-sm" />
//             </div>
//             <h3 className={`font-semibold text-sm ${dark ? 'text-white' : 'text-amber-900'}`}>
...
//         {/* Content Area (Scrollable if multiple anomalies exist) */}
//         <div className="flex-1 overflow-y-auto p-4 space-y-6">
//           {anomalies.map((anomaly, idx) => {
//             // Strategic Calculation: Suggesting a downgraded or regional realistic tier option to user
...
//                     <span className="text-xs bg-red-100 dark:bg-red-950/50 text-red-600 dark:text-red-400 font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
//                       <MdTrendingUp /> +{anomaly.percentageIncrease.toFixed(0)}%
//                     </span>
//                   </div>
...
//                       <p className={`text-sm font-semibold ${dark ? 'text-white' : 'text-gray-900'}`}>₹{anomaly.oldPrice}</p>
//                     </div>
//                     <div>
//                       <p className="text-gray-400">New Detected Bill</p>
//                       <p className={`text-sm font-bold ${dark ? 'text-red-500' : 'text-red-600'}`}>₹{anomaly.newPrice}</p>
//                     </div>
//                   </div>
//                 </div>

//                 {/* Reduced/Target Smart Membership Recommendation */}
//                 <div className={`p-4 rounded-xl border border-dashed ${dark ? 'bg-emerald-950/10 border-emerald-900/50' : 'bg-emerald-50/50 border-emerald-200'}`}>
//                   <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider flex items-center gap-1 mb-1">
//                     <MdLocalOffer /> Retention Strategy Target
//                   </p>
//                   <p className={`text-xs ${dark ? 'text-gray-400' : 'text-gray-600'}`}>
//                     We recommend pushing their retention desk down to your historical rate of <span className="font-bold text-emerald-500">₹{suggestedReducedPrice}</span> or downgrading to their base tier structure.
//                   </p>
//                 </div>

//                 {/* Action Blueprint / Email Interface */}
//                 <div>
//                   {!emailDrafts[anomaly.merchant] ? (
//                     <button 
//                       onClick={() => handleGenerateEmail(anomaly)}
//                       disabled={generating == anomaly.merchant}
//                       className="w-full flex items-center justify-center gap-2 text-xs font-semibold bg-amber-500 hover:bg-amber-600 text-white px-4 py-2.5 rounded-xl transition-all disabled:opacity-50"
//                     >
//                       <MdAutoFixHigh />
...
//                       <p className="text-xs font-semibold mb-1 text-amber-500 uppercase tracking-wider flex items-center gap-1">
//                         <MdAutoFixHigh /> Copy & Deploy to Support
//                       </p>
//                       <div className={`p-3 rounded-xl text-xs whitespace-pre-wrap font-medium h-48 overflow-y-auto border
//                         ${dark ? 'bg-[#0A0A0A] text-gray-300 border-[#2A2A2A]' : 'bg-white text-gray-700 border-gray-200'}`}>
//                         {emailDrafts[anomaly.merchant]}
//                       </div>
//                       <button 
//                         onClick={() => handleCopy(anomaly.merchant, emailDrafts[anomaly.merchant])}
//                         className="absolute top-7 right-2 p-1.5 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-md transition-colors"
//                       >
//                         {copied === anomaly.merchant ? <MdCheck className="text-green-500" /> : <MdContentCopy className={dark ? "text-gray-400" : "text-gray-500"} />}
//                       </button>
//                     </div>
//                   )}
//                 </div>
              ...
              <div key={idx} className="space-y-4">
                ...
                <div className={`p-4 rounded-xl border ${dark ? 'bg-[#1E1E1E] border-[#2A2A2A]' : 'bg-gray-50 border-gray-100'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
                    ...
                    <div>
                      <p className="text-gray-400">New Detected Bill</p>
                      <p className="text-sm font-bold text-red-500">₹{anomaly.newPrice}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className={`p-4 rounded-xl border border-dashed ${dark ? 'bg-teal-950/20 border-teal-900/50' : 'bg-teal-50/50 border-teal-200'}`}>
                  ...
                  <p className={`text-xs ${dark ? 'text-gray-400' : 'text-gray-600'}`}>
                    We recommend pushing their retention desk down to your historical rate of <span className="font-bold text-teal-600 dark:text-teal-400">₹{suggestedReducedPrice}</span> or downgrading to their base tier structure.
                  </p>
                </div>
                ...
                <div>
                  {!emailDrafts[anomaly.merchant] ? (
                    ...
                    <div className="relative mt-2">
                      <p className="text-xs font-semibold mb-1 text-teal-600 dark:text-teal-400 uppercase tracking-wider flex items-center gap-1">
                        <MdAutoFixHigh /> Copy & Deploy to Support
                      </p>
                      <div className={`p-3 rounded-xl text-xs whitespace-pre-wrap font-medium h-48 overflow-y-auto border shadow-inner
                        ${dark ? 'bg-[#0A0A0A] text-gray-300 border-[#2A2A2A]' : 'bg-white text-gray-700 border-gray-200'}`}>
                        {emailDrafts[anomaly.merchant]}
                      </div>
                      <button 
