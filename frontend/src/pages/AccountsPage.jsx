...
<div className="flex items-center justify-between mb-6">
  <div className="flex items-center gap-3">
    <div className={`w-8 h-8 ${iconBox(dark)}`}>
      <MdAccountBalance className={`text-[15px] ${dark ? 'text-[#555]' : 'text-[#AAA]'}`} />
    </div>
    <div>
      <h1 className={`text-[25px] font-bold leading-none ${text(dark)}`}>Accounts</h1>
      <p className={`text-[11px] mt-0.5 ${subtext(dark)}`}>
        Net Worth:
        <span className={`font-semibold ml-1 ${netWorth >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
  ...
  <button onClick={openCreate}
    ...
    <MdAdd className="text-sm" /> Add Account
  </button>
</div>
