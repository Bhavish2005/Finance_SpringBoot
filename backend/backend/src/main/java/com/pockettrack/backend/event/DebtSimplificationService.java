...
public class DebtSimplificationService {
    ...
    private final SharedExpenseRepository sharedExpenseRepository;
    private final ExpenseSplitRepository expenseSplitRepository;
    private final SettlementRepository settlementRepository; // <-- NEW INJECTION!
    ...
    public List<DebtSummary> calculateSimplifiedDebts(UUID eventId, Set<User> participants) {
        ...
        // 1. Add credit for what people paid (Expenses)
        List<SharedExpense> expenses = sharedExpenseRepository.findByEventId(eventId);
        for (SharedExpense expense : expenses) {
            UUID payerId = expense.getPaidBy().getId();
            netBalances.put(payerId, netBalances.get(payerId).add(expense.getTotalAmount()));
        ...
        // 2. Subtract debit for what people owe (Splits)
        List<ExpenseSplit> splits = expenseSplitRepository.findByEventId(eventId);
        for (ExpenseSplit split : splits) {
            UUID debtorId = split.getOwedBy().getId();
            netBalances.put(debtorId, netBalances.get(debtorId).subtract(split.getAmountOwed()));
        ...
        // 3. THE FIX: Apply adjustments for money already sent! (Settlements)
        List<Settlement> settlements = settlementRepository.findByEventId(eventId);
        for (Settlement s : settlements) {
            UUID payerId = s.getPayer().getId(); // The person paying off their debt
            UUID payeeId = s.getPayee().getId(); // The person receiving the money

            // The payer's balance goes UP (closer to 0) because they are clearing their negative debt
            netBalances.put(payerId, netBalances.get(payerId).add(s.getAmount()));
            // The payee's balance goes DOWN because they received what they were owed
            netBalances.put(payeeId, netBalances.get(payeeId).subtract(s.getAmount()));
        }

        // 4. Separate into Debtors and Creditors heaps (Ignore 0 balances)
        PriorityQueue<Map.Entry<UUID, BigDecimal>> debtors = new PriorityQueue<>(Map.Entry.comparingByValue());
        PriorityQueue<Map.Entry<UUID, BigDecimal>> creditors = new PriorityQueue<>((a, b) -> b.getValue().compareTo(a.getValue()));

        for (Map.Entry<UUID, BigDecimal> entry : netBalances.entrySet()) {
            // Only add to queues if the balance isn't effectively zero
