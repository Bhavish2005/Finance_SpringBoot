...
public class SubscriptionSentinelService {
    ...
    public List<AnomalyAlert> detectStealthIncreases(User user) {
        // Just fetch the pre-calculated rows from the database! Lightning fast. ⚡
        List<AnomalyAlertEntity> entities = anomalyAlertRepository
                .findByUserIdAndIsResolvedFalseOrderByDateDetectedDesc(user.getId());
        
        // Map them to the DTO your frontend already expects
        return entities.stream().map(e -> new AnomalyAlert(
                e.getMerchant(),
                e.getOldPrice(),
                e.getNewPrice(),
                e.getPercentageChange(),
                e.getDateDetected().toString(),
                e.getType(),
                e.isRead()
        )).collect(Collectors.toList());
    }
    public void markAllAsRead(User user) {
        ...
        anomalyAlertRepository.saveAll(entities);
    }
}